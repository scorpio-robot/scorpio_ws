...
DriveWidget::DriveWidget(QWidget *parent)
    : QWidget(parent), linear_velocity_(0), angular_velocity_(0),
      linear_scale_(1), angular_scale_(1), x_mouse_(0), y_mouse_(0),
...
void DriveWidget::sendVelocitiesFromMouse(int x, int y, int width, int height) {
  ...
  x_mouse_ = x;
  if (x_mouse_ < width / 2 - size / 2)
    x_mouse_ = width / 2 - size / 2;
  else if (x_mouse_ > width / 2 + size / 2)
    x_mouse_ = width / 2 + size / 2;
  y_mouse_ = y;
  if (y_mouse_ < height / 2 - size / 2)
    y_mouse_ = height / 2 - size / 2;
  else if (y_mouse_ > height / 2 + size / 2)
    y_mouse_ = height / 2 + size / 2;
  ...
  if (linear_velocity_ < -1.0)
    ...
    linear_velocity_ = 1.0;
  if (fabs(linear_velocity_) < 0.1)
    linear_velocity_ = 0;
  angular_velocity_ =
      (1.0 - float(x - hpad) / float(size / 2)) * angular_scale_;
  if (angular_velocity_ < -1.0)
    angular_velocity_ = -1.0;
  else if (angular_velocity_ > 1.0)
    angular_velocity_ = 1.0;
