...
TeleopPanel::TeleopPanel(QWidget *parent)
    : rviz_common::Panel(parent), linear_velocity_(0), angular_velocity_(0),
      mouse_pressed_(false), mouse_pressed_sent_(false) {
  ...
  connect(drive_widget_, SIGNAL(outputVelocity(float, float, bool)), this,
          SLOT(setVel(float, float, bool)));
  connect(output_timer, SIGNAL(timeout()), this, SLOT(sendVel()));

  output_timer->start(100);

  velocity_publisher_ =
      node_->create_publisher<sensor_msgs::msg::Joy>("/joy", 5);
  attemptable_publisher_ =
      node_->create_publisher<std_msgs::msg::Bool>("/planning_attemptable", 5);
  update_publisher_ = node_->create_publisher<std_msgs::msg::Bool>(
      "/update_visibility_graph", 5);
  reset_publisher_ = node_->create_publisher<std_msgs::msg::Empty>(
      "/reset_visibility_graph", 5);
  read_publisher_ =
      node_->create_publisher<std_msgs::msg::String>("/read_file_dir", 5);
  save_publisher_ =
      node_->create_publisher<std_msgs::msg::String>("/save_file_dir", 5);
...
void TeleopPanel::pressButton2() {
  if (rclcpp::ok() && velocity_publisher_->get_subscription_count() > 0) {
    ...
    joy.header.stamp =
        node_->now(); // ROS2 uses node_->now() to get the current time
...
void TeleopPanel::pressButton3() {
  if (rclcpp::ok() && read_publisher_->get_subscription_count() > 0) {
    QString qFilename = QFileDialog::getOpenFileName(
        this, tr("Read File"), "/", tr("VGH - Visibility Graph Files (*.vgh)"));
...
void TeleopPanel::pressButton4() {
  if (rclcpp::ok() && save_publisher_->get_subscription_count() > 0) {
    QString qFilename = QFileDialog::getSaveFileName(
        this, tr("Save File"), "/", tr("VGH - Visibility Graph Files (*.vgh)"));
    ...
    if (filename != "") {
      ...
      if (length < 4) {
        filename += ".vgh";
      } else if (filename[length - 4] != '.' || filename[length - 3] != 'v' ||
                 filename[length - 2] != 'g' || filename[length - 1] != 'h') {
        filename += ".vgh";
      }
    }
    std_msgs::msg::String msg;
...
void TeleopPanel::sendVel() {
  if (rclcpp::ok() && velocity_publisher_->get_subscription_count() > 0 &&
      (mouse_pressed_ || mouse_pressed_sent_)) {
    sensor_msgs::msg::Joy joy;
